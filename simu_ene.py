...
def simulazione_1e0g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    sciame = [Elettrone(energia_iniziale, altezza_sciame)]
...
def simulazione_1f0g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    sciame = [Fotone(energia_iniziale, altezza_sciame)]
    ...
    return len(particelle_rilevate)
def simulazione_1e5g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    for i in tqdm(range(t)):        #print('Lunghezza sciame:', len(sciame))
    ...
    return len(particelle_rilevate)
def simulazione_1f5g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1e10g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1f10g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1e15g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1f15g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1e20g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1f20g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1e25g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1f25g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1e30g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1f30g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1e35g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1f35g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1e40g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1f40g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1e45g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
+        int: Il numero di particelle che sono state rilevate.
    ...
    return len(particelle_rilevate)
def simulazione_1f45g(s, energia_iniziale, altezza_sciame, altezza_detector):
    """
+    Funzione che simula l'andamento di un elettrone con angolo 0
+    prima di arrivare al detector con e_iniziale=10^6
+
...
for m in range(num_simu+1):
    print(m)
    energia_iniziale=1000000+2*m*1000000
    risu_1e0g.append(simulazione_1e0g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1f0g.append(simulazione_1f0g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1e5g.append(simulazione_1e5g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1f5g.append(simulazione_1f5g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1e10g.append(simulazione_1e10g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1f10g.append(simulazione_1f10g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1e15g.append(simulazione_1e15g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1f15g.append(simulazione_1f15g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1e20g.append(simulazione_1e20g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1f20g.append(simulazione_1f20g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1e25g.append(simulazione_1e25g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1f25g.append(simulazione_1f25g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1e30g.append(simulazione_1e30g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1f30g.append(simulazione_1f30g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1e35g.append(simulazione_1e35g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1f35g.append(simulazione_1f35g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1e40g.append(simulazione_1e40g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1f40g.append(simulazione_1f40g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1e45g.append(simulazione_1e45g(s, energia_iniziale, altezza_sciame, altezza_detector))
    risu_1f45g.append(simulazione_1f45g(s, energia_iniziale, altezza_sciame, altezza_detector))
    '''
df_risu_1e0g = pd.DataFrame(risu_1e0g, columns=["Simulazione 1e-, angolo=0"])
percorso_file = "simulazione_ene_1e0g.csv"
df_risu_1e0g.to_csv(percorso_file, index=False)

df_risu_1f0g = pd.DataFrame(risu_1f0g, columns=["Simulazione 1f, angolo=0"])
percorso_file = "simulazione_ene_1f0g.csv"
...
percorso_file = "simulazione_1e25g.csv"
